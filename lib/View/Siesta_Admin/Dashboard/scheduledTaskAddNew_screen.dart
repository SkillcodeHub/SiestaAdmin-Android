...
class ScheduledTaskAddNewPage extends StatefulWidget {
  const ScheduledTaskAddNewPage({super.key});

  @override
  State<ScheduledTaskAddNewPage> createState() =>
      _ScheduledTaskAddNewPageState();
}

class _ScheduledTaskAddNewPageState extends State<ScheduledTaskAddNewPage> {
...
class _ScheduledTaskAddNewBodyState extends State<ScheduledTaskAddNewBody> {
  ...
  Widget build(BuildContext context) {
    ...
    return ChangeNotifierProvider<GetActivitySchedulersListViewmodel>.value(
            ...
            case Status.ERROR:
              if (value.getActivitySchedulersList.message ==
                ...
                return ErrorScreenWidget(
                  onRefresh: () async {},
                  loadingText:
                      value.getActivitySchedulersList.message.toString(),
                );
              } else {
                return Center(
                  child:
                      Text(value.getActivitySchedulersList.message.toString()),
                );
              }

            case Status.COMPLETED:
              List<ActivitySchedulers>? list =
                  value.getActivitySchedulersList.data!.data!;
                    ...
                    switch (value.getAssetsList.status!) {
                      ...
                      case Status.ERROR:
                        if (value.getAssetsList.message ==
                          ...
                          return Center(
                            child: Text(value.getAssetsList.message.toString()),
                          );
                        }
                            ...
                            builder: (context, value, _) {
                              switch (value.activityGroupsList.status!) {
                                ...
                                case Status.COMPLETED:
                                  ...
                                  _propertyList!.clear();
                                  List<Assets> list1 = getAssetsListViewmodel
                                      .getAssetsList.data!.data!;
